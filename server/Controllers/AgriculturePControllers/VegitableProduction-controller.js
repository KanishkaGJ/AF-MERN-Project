...
const addVegitableProduction = async (req, res, next) => {
    ...
    newVegitableProduction.save().then(() => {
...
const getAllVegitableProduction = async (req, res, next) => {
    VegitableProduction.find().then((vegitableproduction) => {
...
const getVegitableProductionByID = async (req, res) => {
    ...
    const production = await VegitableProduction.findById(vegitableProductionID).then((production) => {
        res.status(200).send({ status: "Vegitable production is fetched", production});
    })
    .catch((error) => {
...
const updateVegitableProduction = async (req, res, next) => {
    ...
    const updateProduction = await VegitableProduction.findOneAndUpdate(vegitableProductionID, updateVegitableProduction).then(() => {
        res.status(200).send({ status: "Vegitable production is updated successfully!!"})
    })
    .catch((error) => {
        console.log(error);
...
const deleteVegitableProduction  = async (req, res, next) => {
    ...
    await VegitableProduction.findOneAndDelete(vegitableProductionID).then(() => {
        res.status(200).send({ status: "vegitable production have been deleted successfully!!"});
    })
    .catch((error) => {
        console.log(error.message);
        res.status(500).send({ status: "Error when deleting the vegitable production", error: error.message});
    });
};
